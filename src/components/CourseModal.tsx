    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/75 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[92vh] overflow-hidden flex flex-col border border-slate-200">
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-slate-800 text-sm">
          {/* Course Banner Image if available */}
          {course.image && (
            <div className="relative h-48 sm:h-56 w-full rounded-xl overflow-hidden border border-slate-200 shadow-xs bg-slate-900">
              <img
                src={course.image}
              ...
              <div className="absolute top-3 left-3 pointer-events-none">
                <span className="px-2.5 py-1 bg-orange-500 text-white rounded-md text-[10.5px] font-bold uppercase tracking-wider shadow-xs">
                  {course.categoryLabel}
                </span>
              </div>
              <div className="absolute bottom-3 left-4 right-4 pointer-events-none">
                <span className="text-xs text-white font-medium drop-shadow-xs line-clamp-1">
                  {course.imageAlt || course.title}
                </span>
              </div>
            </div>
          )}
